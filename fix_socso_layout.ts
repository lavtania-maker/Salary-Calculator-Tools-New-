...
if (socsoSeoIndex !== -1) {
  ...
  chunk = chunk.replace(
    /<div\s+style="\s*display:\s*grid;\s*grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(220px,\s*1fr\)\);\s*gap:\s*16px;\s*"\s*>/g,
    '<div class="content-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px;">',
  );

  // Replace back
  content =
    content.substring(0, socsoSeoIndex) +
    chunk +
    content.substring(socsoEndIndex);
}

// 3. Fix the SOCSO result card.
content = content.replace(
  /id="socsoResultCard"/,
  'class="card" id="socsoResultCard"',
);

let socsoTitleRe =
  /<div\s+style="\s*display:\s*flex;\s*justify-content:\s*space-between;\s*align-items:\s*center;\s*margin-bottom:\s*0\.75rem;\s*"\s*>\s*<h2\s+class="card-title"\s+style="margin-bottom:\s*0;\s*font-size:\s*1\.1rem"\s*>\s*SOCSO Results\s*<\/h2>\s*<\/div>/g;
content = content.replace(
  socsoTitleRe,
  '<h2 class="card-title">SOCSO Results</h2>',
);

let socsoContentRe =
  /id="socsoResultsContent"\s*class="results-content"\s*style="display:\s*none;\s*padding:\s*0"/g;
content = content.replace(
  socsoContentRe,
  'id="socsoResultsContent" class="result-content" style="display: none;"',
);
...
let socsoBoxRe =
  /<div\s*style="\s*background:\s*#fff;\s*border:\s*1px\s*solid\s*#e2e8f0;\s*border-radius:\s*16px;\s*padding:\s*20px;\s*box-shadow:\s*0\s*1px\s*3px\s*0\s*rgba\(0,\s*0,\s*0,\s*0\.1\);\s*"\s*>/g;
content = content.replace(socsoBoxRe, '<div class="result-section">');

let socsoEmpRowRe =
  /<div\s*style="\s*display:\s*flex;\s*align-items:\s*center;\s*justify-content:\s*space-between;\s*padding:\s*12px\s*0;\s*border-bottom:\s*1px\s*solid\s*#f1f5f9;\s*"\s*>/g;
...
content = content.replace(
  /<span\s*id="socsoEmployeeCardVal"\s*style="[^"]*"\s*>/g,
  '<span id="socsoEmployeeCardVal">',
);
content = content.replace(
  /<span\s*id="socsoEmployerCardVal"\s*style="[^"]*"\s*>/g,
  '<span id="socsoEmployerCardVal">',
);
content = content.replace(
  /<span\s*id="socsoTotalCardVal"\s*style="[^"]*"\s*>/g,
  '<span id="socsoTotalCardVal">',
);

// For total cost row
let socsoTotalRowRe =
  /<div\s*style="\s*display:\s*flex;\s*align-items:\s*center;\s*justify-content:\s*space-between;\s*padding-top:\s*16px;\s*margin-top:\s*4px;\s*"\s*>/g;
