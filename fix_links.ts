...
const replaceLinks = (content: string) => {
  return content.replace(/href="\/pcb-income-tax\.html"/g, 'href="/pcb-income-tax"')
                .replace(/href="\/epf-kwsp\.html"/g, 'href="/epf-kwsp"')
                .replace(/href="\/annual-leave-calculator\.html"/g, 'href="/annual-leave-calculator"');
};
...
const processFile = (filePath: string) => {
  ...
  newContent = newContent.replace(/window\.location\.href = "\/pcb-income-tax\.html"/g, 'window.location.href = "/pcb-income-tax"');
  ...
  const badNavLine = '<a href="/annual-leave-calculator" class="footer-link">Annual Leave Calculator</a>';
  const goodNavLine = '<a href="/annual-leave-calculator" class="nav-item">Annual Leave Calculator</a>';
  const badNavLineUnreplaced = '<a href="/annual-leave-calculator" class="footer-link">Annual Leave Calculator</a>';
...
const processDir = (dirPath: string) => {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      if (!fullPath.includes('node_modules') && !fullPath.includes('.git') && !fullPath.includes('dist') && !fullPath.includes('public')) {
        processDir(fullPath);
      }
    } else {
